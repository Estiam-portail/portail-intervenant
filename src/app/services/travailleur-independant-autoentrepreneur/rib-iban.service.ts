...
import { FileUpload } from 'src/app/models/travaillleur-independant/file-upload-rib-iban';
import { GetDocumentsTravailleurIndependantService } from '../get-documents-travailleur-independant.service';

@Injectable({
  providedIn: 'root'
})
export class RibIbanService {
    ...
    uploadTask.snapshotChanges().pipe(
      finalize(() => {
        storageRef.getDownloadURL().subscribe(downloadURL => {
          ...
          this.getDocumentsTravailleurIndependantService.documents.UrlRIBIBAN = fileUpload.url
        });
      })
    ).subscribe();
